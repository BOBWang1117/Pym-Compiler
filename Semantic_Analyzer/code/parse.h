...
typedef enum {DEFAULT_EXPR,OP_EXPR, CONST_EXPR, ID_EXPR,ARRAY_EXPR, CALL_EXPR, CALL_EXPR_ARGS, ASSIGN_EXPR} ExpressionKind;
typedef enum {DEFAULT_TYPE,INT_TYPE, NUM_TYPE,STR_TYPE,BOOL_TYPE,ARRAY_TYPE} ExprType;
...
public:
    ...
    TreeNode(DclKind, ExprType, string);//declare
    TreeNode(DclKind,ExprType,string ,int);//array declare
...
void printTree(TreeNode*, bool* status);
void printTree(TreeNode*,int indent, bool* status);
