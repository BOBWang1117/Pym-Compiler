...
void printTree(TreeNode* tree,int indent) {
	switch (tree->nodeKind)
	{
		case PROGRAM_NO: {
			...
			{
				printTree(tree->rSibling,indent);
				tree = tree->rSibling;
			}
			break;
		}
		...
		case DCL_ND: {
			...
			case FUN_DCL: {
				...
				if (tree->child[0]->nodeKind== PRAMA_LIST_ND) {
					...
					while (param!=NULL)
					{
						printTree(param,indent+1);
						param = param->rSibling;
					}

					//compound
					printTree(tree->child[1], indent + 1);
				}
				if (tree->child[0]->nodeKind == CPD_ND) {
					cout <<"  (At Line "<<tree->lineNum<<")"<<endl;
					printTree(tree->child[0], indent + 1);
				}
				break;
			}
			case VAR_DCL: {
		...
		case IF_ND: {
			...
			if (tree->child[2] != NULL) {
				printTree(tree->child[2], indent + 1);
			}

			//else stmt
			if (tree->child[3] != NULL) {
				printTree(tree->child[3], indent + 1);
			}
			break;
		}
		case ELIF_ND: {
